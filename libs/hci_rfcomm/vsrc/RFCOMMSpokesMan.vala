...
public class hciplus.RFCOMMSpokesMan : hciplus.HCIScribe {
	enum RFCOMMCommand {
		CREATE_CONNECTION = 0x05,
		LE_CREATE_CONNECTION = 0x0d,
		RFCOMM_PACKET_TYPE = 0x0008 | 0x0010 | 0x0400 | 0x0800 | 0x4000 | 0x8000,
	}
	...
	public void LEConnect(BluetoothDevice to) {
		...
		pushCommand(HCISpokesMan.HCICommandType.HCI_LINK_CONTROL, RFCOMMCommand.LE_CREATE_CONNECTION, &pkt);
		//hos.writeCommand(HCISpokesMan.HCICommandType.HCI_LINK_CONTROL, RFCOMMCommand.LE_CREATE_CONNECTION, &pkt);
	}

	public void RFCOMMConnect(BluetoothDevice to) {
#if false
typedef struct {
	bdaddr_t	bdaddr;
	uint16_t	pkt_type;
	...
	uint8_t		role_switch;
		...
		pkt.concat_char(to.rawaddr[4]); // bluetooth address
		pkt.concat_char(to.rawaddr[5]); // bluetooth address
		aroop_uword16 ptype = RFCOMMCommand.RFCOMM_PACKET_TYPE;
		ptype = 0xcc18;
		...
		print("Connect .. %X %X %X \n"
			...
			, to.clock_offset);
#endif
		pushCommand(HCISpokesMan.HCICommandType.HCI_LINK_CONTROL, RFCOMMCommand.CREATE_CONNECTION, &pkt);
		//hos.writeCommand(HCISpokesMan.HCICommandType.HCI_LINK_CONTROL, RFCOMMCommand.CREATE_CONNECTION, &pkt);
	}
}
